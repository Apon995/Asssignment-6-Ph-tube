const getCatagories = async () => {
  try {
    ...
    data.data.forEach((element) => {
      ...
      button.innerHTML = `
        <button class=" bg-[#25252533] text-[#252525] focus:bg-[#FF1F3D] focus:text-white font-medium rounded-md px-4 py-2" onclick="getCatagoryId('${element?.category_id
        }'); toggleSpinner('${true}')" >${element.category || "no available"
        }</button>
          `;
  ...
  try {
    ...
    if (data?.status === true) {
      (byView !== undefined ? byView : data.data).forEach((element) => {
        ...
        div.innerHTML = `
                  <img src="${element.thumbnail || "https://i.ibb.co/DRxB1Wm/sunris.jpg"
          }" alt="" class="md:w-[315px] md:h-[200px] w-[380px]  mx-auto">
                      <span id='showtime' class="bg-black text-white ${element.others.posted_date == "" ? "hidden" : "block"
          ...
          "https://i.ibb.co/DRxB1Wm/sunris.jpg"
                     ...
                     <h1 class ="text-xl font-medium text-[#171717]">${element.title || "no title availble"
          }</h1>
                  </div>
                  <p class="pl-12 inline-flex gap-1 text-[#171717b3] text-sm font-normal pb-1"> ${element.authors[0].profile_name || "not found author"
          } ${element.authors[0].verified || "" == true
            ? `<img src="./assets/verified.svg">`
            : ""
          } </p>
                  <p class="pl-12 text-[#171717b3] text-sm font-normal">${element.others.views || "0"
          }Views</p>
      ...
      div.innerHTML = `
              ...
              <h1 class="font-bold text-black text-2xl ">${data.message || "no data found !"
        }</h1>
